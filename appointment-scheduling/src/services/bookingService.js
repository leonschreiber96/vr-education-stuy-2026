...
class BookingService {
    ...
    static createBooking(participantId, timeslotId, isFollowup = false) {
        ...
        if (!participantId || !timeslotId) {
            throw new ValidationError('Participant ID and Timeslot ID are required');
        }
        ...
        if (isFollowup) {
            if (timeslot.appointment_type === 'primary') {
                throw new ConflictError('Cannot book a follow-up in a primary-only timeslot');
            }
        } else {
            if (timeslot.appointment_type === 'followup') {
                throw new ConflictError('Cannot book a primary appointment in a follow-up-only timeslot');
            }
        }
        ...
        if (isFollowup) {
            ...
            if (hasFollowup) {
                throw new ConflictError('Participant already has a follow-up booking');
            }

            // Must have a primary booking first
            const hasPrimary = existingBookings.some(b => !b.is_followup);
            if (!hasPrimary) {
                throw new ConflictError('Must book a primary appointment before booking a follow-up');
            }
            ...
            const daysDifference = Math.floor((followupDate - primaryDate) / (1000 * 60 * 60 * 24));

            if (daysDifference < env.FOLLOWUP_MIN_DAYS || daysDifference > env.FOLLOWUP_MAX_DAYS) {
                throw new ConflictError(
                    `Follow-up must be ${env.FOLLOWUP_MIN_DAYS}-${env.FOLLOWUP_MAX_DAYS} days after primary appointment. Current gap: ${daysDifference} days.`
                );
            }
        } else {
            const hasPrimary = existingBookings.some(b => !b.is_followup);
            if (hasPrimary) {
                throw new ConflictError('Participant already has a primary booking');
            }
        }
        ...
        NotificationService.sendBookingConfirmation(booking, participant, timeslot)
            .catch(err => {
    ...
    static cancelBooking(bookingId, reason = '') {
        ...
        if (participant && timeslot) {
            NotificationService.sendBookingCancellation(booking, participant, timeslot, reason)
                .catch(err => {
     ...
     */
    static cancelTimeslotBookings(timeslotId, reason = 'Timeslot was cancelled by administrator') {
        const timeslot = db.getTimeslotById(timeslotId);
        if (!timeslot) {
            throw new NotFoundError('Timeslot not found');
        }
    ...
    static updateBookingResultStatus(bookingId, resultStatus) {
        ...
        const updatedBooking = db.updateBookingResultStatus(bookingId, resultStatus);
    ...
    static validateFollowupBooking(participantId, followupTimeslotId) {
        ...
        if (!primaryBooking) {
            return {
                isValid: false,
                message: 'Must have a primary appointment before booking a follow-up',
            };
        }
        ...
        if (daysDifference < env.FOLLOWUP_MIN_DAYS || daysDifference > env.FOLLOWUP_MAX_DAYS) {
            return {
                isValid: false,
                message: `Follow-up must be ${env.FOLLOWUP_MIN_DAYS}-${env.FOLLOWUP_MAX_DAYS} days after primary appointment. Current gap: ${daysDifference} days.`,
                daysDifference,
            };
        }
